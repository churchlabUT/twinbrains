...
for (i in 1:length(group)){ 
  ...
  # SUB
  for (sub in subdirs){
    model.dirs = Sys.glob(sprintf("%s/model/CF/Run?.feat", sub))
...
# create one large df
  ...
  write.csv(x=all.subs, file='CF_all_desmat.csv')
  vif.subs = all.subs[(all.subs$value > 10 & is.na(all.subs$value)==FALSE),]
  write.csv(x=vif.subs, file='CF_VIFgreaterthan10.csv')
...
#  write.csv(x=all.empty, file='CF_empty_ev_subs.csv')
